...
class BuildingControllerTest {
    ...
    void shouldChangeStatus_whenAuthorized() throws Exception {
        ...
        verify(buildingService, times(1)).changeBuildingStatus(eq(buildingId), eq(BuildingStatus.INACTIVE), any());
    }

    @Test
    void shouldReturn404_whenChangeStatusInvalid() throws Exception {
        Mockito.when(authzService.canUpdateBuilding()).thenReturn(true);
        Mockito.doThrow(new IllegalArgumentException("bad")).when(buildingService).changeBuildingStatus(eq(buildingId),
    ...
    void shouldCreateDeletionRequest_whenAuthorized() throws Exception {
        ...
        var dto = new BuildingDeletionRequestDto(UUID.randomUUID(), buildingId, UUID.randomUUID(), "reason", null, null,
                com.QhomeBase.baseservice.model.BuildingDeletionStatus.PENDING, OffsetDateTime.now(), null);
    ...
    void shouldApproveDeletionRequest_whenAuthorized() throws Exception {
        ...
        var dto = new BuildingDeletionRequestDto(requestId, buildingId, UUID.randomUUID(), "r", UUID.randomUUID(),
                "note", com.QhomeBase.baseservice.model.BuildingDeletionStatus.APPROVED, OffsetDateTime.now(),
                OffsetDateTime.now());
    ...
    void shouldRejectDeletionRequest_whenAuthorized() throws Exception {
        ...
        UUID requestId = UUID.randomUUID();
        var dto = new BuildingDeletionRequestDto(requestId, buildingId, UUID.randomUUID(), "r", UUID.randomUUID(),
                "note", com.QhomeBase.baseservice.model.BuildingDeletionStatus.REJECTED, OffsetDateTime.now(),
                OffsetDateTime.now());
    ...
    void shouldGetPendingDeletionRequests_whenAuthorized() throws Exception {
        ...
        var dto = new BuildingDeletionRequestDto(UUID.randomUUID(), buildingId, UUID.randomUUID(), "r", null, null,
                com.QhomeBase.baseservice.model.BuildingDeletionStatus.PENDING, OffsetDateTime.now(), null);
    ...
    void shouldReturn404_whenGetBuildingDeletionRequestNotFound() throws Exception {
        UUID requestId = UUID.randomUUID();
        Mockito.when(buildingDeletionService.getById(eq(requestId))).thenThrow(new IllegalArgumentException("nf"));
    ...
    void shouldGetMyDeletingBuildingsRaw() throws Exception {
        ...
        mockMvc.perform(get("/api/buildings/my-deleting-buildings-raw").with(securityContext(ctx)).principal(auth))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$", hasSize(1)))
    ...
    void shouldGetTargetsStatus() throws Exception {
        Mockito.when(buildingDeletionService.getBuildingDeletionTargetsStatus(eq(buildingId))).thenReturn(Map.of(
