...
class ResidentControllerTest {
    ...
    void setUp() {
        ...
        authResident = new UsernamePasswordAuthenticationToken(principalResident, null, List.of(new SimpleGrantedAuthority("ROLE_RESIDENT")));

        var principalAdmin = new UserPrincipal(UUID.randomUUID(), "admin", List.of("ADMIN"), List.of(), "token");
        authAdmin = new UsernamePasswordAuthenticationToken(principalAdmin, null, List.of(new SimpleGrantedAuthority("ROLE_ADMIN")));
    }

    @Test
    void shouldGetResidentsWithoutAccount() throws Exception {
        UUID unitId = UUID.randomUUID();
        var dto = new ResidentWithoutAccountDto(UUID.randomUUID(), "John", "0123", "john@example.com", "NID", LocalDate.of(1990,1,1), ResidentStatus.ACTIVE, "OWNER", true);
        Mockito.when(residentAccountService.getResidentsWithoutAccount(eq(unitId), eq(userId))).thenReturn(List.of(dto));
    ...
    void shouldCreateAccountRequest() throws Exception {
        ...
        Mockito.when(residentAccountService.createAccountRequest(any(CreateAccountRequestDto.class), eq(userId))).thenReturn(result);
    ...
    void shouldGetResidentById() throws Exception {
        ...
        var dto = new ResidentDto(residentId, "John", "0123", "john@example.com", "NID", LocalDate.of(1990,1,1), ResidentStatus.ACTIVE, userId, java.time.OffsetDateTime.now(), java.time.OffsetDateTime.now());
    ...
    void shouldGetMyUnits() throws Exception {
        UUID unitId = UUID.randomUUID();
        var unit = new UnitDto(unitId, UUID.randomUUID(), "B-01", "Tower", "U-101", 10, java.math.BigDecimal.valueOf(80), 2, com.QhomeBase.baseservice.model.UnitStatus.ACTIVE, null, java.time.OffsetDateTime.now(), java.time.OffsetDateTime.now());
    ...
    void shouldSyncStaffResident() throws Exception {
        ...
        var dto = new ResidentDto(UUID.randomUUID(), "Staff A", "0123", "staff@example.com", null, null, ResidentStatus.ACTIVE, null, java.time.OffsetDateTime.now(), java.time.OffsetDateTime.now());
