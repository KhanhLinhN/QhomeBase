...
@Slf4j
@RestController
@RequestMapping("/api/elevator-card")
@RequiredArgsConstructor
public class ElevatorCardController {

    private final ElevatorCardRegistrationService service;
    private final VnpayService vnpayService;

    @PostMapping
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ElevatorCardRegistrationResponseDto> register(
            @RequestBody ElevatorCardRegistrationDto dto,
            Authentication authentication) {

        Long userId = getUserIdFromAuthentication(authentication);
        ElevatorCardRegistrationResponseDto result = service.registerElevatorCard(dto, userId);

        log.info("User {} registered elevator card", userId);
        return ResponseEntity.ok(result);
    }

    @GetMapping("/me")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<List<ElevatorCardRegistrationResponseDto>> getByUser(Authentication authentication) {
        Long userId = getUserIdFromAuthentication(authentication);
        List<ElevatorCardRegistrationResponseDto> list = service.getByUserId(userId);
        log.info("User {} fetched their elevator card registrations, count={}", userId, list.size());
        return ResponseEntity.ok(list);
    }

    @GetMapping("/{id}")
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ElevatorCardRegistrationResponseDto> getById(
            @PathVariable Long id,
            Authentication authentication) {
        Long userId = getUserIdFromAuthentication(authentication);
        ElevatorCardRegistrationResponseDto result = service.getById(id, userId);
        log.info("User {} fetched elevator card registration {}", userId, id);
        return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<?> getByUserPaginated(
        ...
        Page<ElevatorCardRegistrationResponseDto> result = service.getByUserIdPaginated(userId, pageIndex, size);

        return ResponseEntity.ok(Map.of(
                "success", true,
                "message", "Danh sách đăng ký thẻ thang máy",
    ...
    public ResponseEntity<?> createVnpayUrlWithData(
        ...
        try {
            log.info("💳 [ElevatorCardController] Tạo VNPAY URL với data cho userId: {}", userId);
            ...
            log.error("❌ [ElevatorCardController] Lỗi tạo VNPAY URL với data: {}", ex);
    ...
    public ResponseEntity<?> handleVnpayReturn(HttpServletRequest request) {
        ...
        log.info("[VNPAY RETURN] ✅ Callback nhận được cho elevator card: {}", params);
        ...
        try {
            ...
            log.info("[VNPAY RETURN] 🔍 Elevator Card Registration ID trích xuất được: {}", registrationId);
            ...
            if (valid && "00".equals(responseCode) && "00".equals(transactionStatus)) {
                ...
                log.info("[VNPAY RETURN] ✅ Elevator Card Registration {} đã được cập nhật sang PAID", registrationId);
    ...
    public ResponseEntity<?> redirectAfterPayment(HttpServletRequest request, HttpServletResponse response) throws java.io.IOException {
        ...
        String deepLinkUrl = "qhomeapp://vnpay-elevator-card-result?registrationId=" + registrationId + "&responseCode=" + responseCode;
    ...
    public ResponseEntity<?> cancelRegistration(
        ...
        try {
            log.info("🗑️ [ElevatorCardController] Hủy registration: {}, userId: {}", id, userId);
            ...
            log.error("❌ [ElevatorCardController] Lỗi hủy registration: {}", id, ex);
            return ResponseEntity.status(ex.getStatusCode()).body(Map.of(
                    "success", false,
                    "message", ex.getReason()
            ));
        } catch (Exception ex) {
            log.error("❌ [ElevatorCardController] Lỗi hệ thống khi hủy registration: {}", ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of(
                    "success", false,
