...
import com.qhomebaseapp.mapper.ResidentCardRegistrationMapper;
import com.qhomebaseapp.mapper.ResidentCardRegistrationResponseMapper;
import com.qhomebaseapp.model.ResidentCardRegistration;
import com.qhomebaseapp.model.User;
import com.qhomebaseapp.repository.UserRepository;
import com.qhomebaseapp.repository.residentcard.ResidentCardRegistrationRepository;
...
@Slf4j
public class ResidentCardRegistrationServiceImpl implements ResidentCardRegistrationService {

    private final ResidentCardRegistrationRepository repository;
    private final UserRepository userRepository;
    private final ResidentCardRegistrationMapper mapper;
    private final ResidentCardRegistrationResponseMapper responseMapper;
    ...
    @Override
    public ResidentCardRegistrationResponseDto registerResidentCard(ResidentCardRegistrationDto dto, Long userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new IllegalArgumentException("User not found with ID: " + userId));

        ResidentCardRegistration entity = mapper.toEntity(dto);
        entity.setUser(user);
        ...
        ResidentCardRegistration saved = repository.save(entity);
        return responseMapper.toDto(saved);
    }

    @Override
    public Map<String, Object> createVnpayPaymentUrlWithData(ResidentCardRegistrationDto dto, Long userId, HttpServletRequest request) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new IllegalArgumentException("User not found with ID: " + userId));
        ...
        log.info("💳 [ResidentCard] Tạo registration {} để thanh toán VNPAY", registrationId);
        ...
        String orderInfo = "Thanh toán phí đăng ký thẻ cư dân #" + registrationId;
        String baseUrl = vnpayProperties.getReturnUrl().replace("/api/invoices/vnpay/redirect", "");
        String returnUrl = baseUrl + "/api/resident-card/vnpay/redirect";
        
        String paymentUrl = vnpayService.createPaymentUrl(registrationId, orderInfo, REGISTRATION_FEE, clientIp, returnUrl);
        
        log.info("💳 [ResidentCard] Tạo VNPAY URL cho registration: {}, userId: {}", registrationId, userId);
    ...
    public void cancelRegistration(Long registrationId, Long userId) {
        ...
        if ("PAID".equalsIgnoreCase(registration.getPaymentStatus())) {
            log.warn("⚠️ [ResidentCard] Không thể hủy registration {} - đã thanh toán thành công", registrationId);
        ...
        log.info("🔄 [ResidentCard] Đã update registration {} thành payment_status UNPAID", registrationId);
    }

    @Override
    public List<ResidentCardRegistrationResponseDto> getByUserId(Long userId) {
        List<ResidentCardRegistration> list = repository.findByUser_Id(userId);
        return list.stream()
    ...
    @Override
    public ResidentCardRegistrationResponseDto getById(Long id, Long userId) {
        ResidentCardRegistration entity = repository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Registration not found"));
    ...
    @Override
    public Page<ResidentCardRegistrationResponseDto> getByUserIdPaginated(Long userId, int page, int size) {
        Page<ResidentCardRegistration> pageResult =
    ...
    public void handleVnpayCallback(Long registrationId, Map<String, String> vnpParams) {
        ...
        log.info("💳 [ResidentCard] VNPAY callback cho registration: {}", registrationId);
        log.info("💳 [ResidentCard] Valid: {}, ResponseCode: {}, TransactionStatus: {}", valid, responseCode, transactionStatus);
        ...
        if (valid && "00".equals(responseCode) && "00".equals(transactionStatus)) {
            ...
            log.info("✅ [ResidentCard] Đã cập nhật registration {} với payment_status PAID", registrationId);
            
            // Gửi email thông báo thanh toán thành công
            try {
                User user = registration.getUser();
                if (user != null && user.getEmail() != null) {
                    String emailSubject = "Thanh toán thành công - Đăng ký thẻ cư dân";
                    ...
                    String emailBody = String.format(
                        "Xin chào %s,\n\n" +
                        "Thanh toán đăng ký thẻ cư dân của bạn đã được xử lý thành công!\n\n" +
                        "Thông tin đăng ký:\n" +
                        "%s" +
                    ...
                    log.info("✅ [ResidentCard] Đã gửi email thông báo thanh toán thành công cho user: {}", user.getEmail());
                }
            } catch (Exception e) {
                log.error("❌ [ResidentCard] Lỗi khi gửi email thông báo thanh toán: {}", e.getMessage(), e);
            }
        } else {
            registration.setPaymentStatus("UNPAID");
            registration.setStatus("PENDING");
            ...
            log.warn("❌ [ResidentCard] Thanh toán thất bại cho registration {} - giữ lại với payment_status UNPAID", registrationId);
